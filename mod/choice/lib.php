 ...
 * @return bool
 */
function mod_choice_core_calendar_validate_event(\calendar_event $event) {
    global $DB;
    ...
    if ($event->eventtype == CHOICE_EVENT_TYPE_OPEN) {
        ...
        if (!empty($record->timeclose) && $event->timestart >= $record->timeclose) {
            throw new \moodle_exception('openafterclose');
        }
        ...
        if (!empty($record->timeopen) && $event->timestart <= $record->timeopen) {
            throw new \moodle_exception('closebeforeopen');
        }
    }
 ...
 * @param \calendar_event $event
 */
function mod_choice_core_calendar_event_updated(\calendar_event $event) {
    global $DB;
