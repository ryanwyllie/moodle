...
class mod_assign_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_get_participant_blind_marking() {
        ...
        $this->assertEquals(false, $result['submitted']);
        $this->assertEquals(false, $result['requiregrading']);
     ...
     */
    public function test_get_participant_summary_only() {
        global $DB;
        $this->resetAfterTest(true);
        ...
        $result = mod_assign_external::get_participant($assignmodule->id, $student->id, true);
        $this->assertEquals($student->id, $result['id']);
        $this->assertEquals(fullname($student), $result['fullname']);
        $this->assertEquals(true, $result['submitted']);
        $this->assertEquals(true, $result['requiregrading']);
        // Make sure we don't get any additional info.
        $this->assertFalse(isset($result['email']));
    }

    /**
     * get_participant should return an expanded list of user details if no summary
     * is requested.
     */
