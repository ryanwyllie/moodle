...
function xmldb_block_timeline_install() {
    ...
    // timeline block without the admin resetting all of the dashboards. Instead we'll
    // just add the timeline block to their dashboards here.
    $sql = "SELECT parentcontextid, subpagepattern
            FROM {block_instances}
            WHERE pagetypepattern = 'my-index'
            AND defaultregion = ?
            AND parentcontextid != ?
            GROUP BY parentcontextid, subpagepattern";
    $params = [BLOCK_POS_RIGHT, $systemcontext->id];
