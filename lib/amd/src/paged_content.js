...
function(
    ...
    var init = function(root, renderPagesContentCallback) {
        ...
        Pages.init(pagesContainer, pubSub, renderPagesContentCallback);

        if (pagingBarContainer.length) {
            PagingBar.init(pagingBarContainer, pubSub);
        }

        if (pagingBarLimitSelectorContainer.length) {
            PagingBarLimitSelector.init(pagingBarLimitSelectorContainer, pubSub);
        }

        if (dropdownContainer.length) {
            Dropdown.init(dropdownContainer, pubSub);
        }
    };
