...
define(['jquery'], function($) {
    ...
    var SELECTORS = {
        ITEM: '[role=treeitem]',
        GROUP: '[role=group],[role=treeitem]:has([role=group]), [role=treeitem][data-requires-ajax=true]',
            ...
            if (item.attr('data-requires-ajax') === 'true' && item.attr('data-loaded') !== 'true') {
                ...
                require([moduleName], function(loader) {
                    ...
                    loader.load(item).done(function() {
                        ...
                        if (item.children('[aria-owns]') !== false) {
                            item.removeAttr('role');
                            item.removeAttr('aria-expanded');
                            item.removeAttr('aria-selected');
                            item.attr('tabindex', -1);
                            item.children('[aria-owns]').first().attr('aria-selected', 'true');
                            item.children('[aria-owns]').first().attr('tabindex', 0);
                        }
                        promise.resolve();
                    });
                });
    ...
    Tree.prototype.finishExpandingGroup = function(item) {
        ...
        var group = owns || item.children(SELECTORS.GROUP);
