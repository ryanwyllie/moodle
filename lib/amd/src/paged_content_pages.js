...
define(
    ...
    var showPages = function(root, pagesData, id, renderPagesContentCallback) {
        ...
        newPagesPromise.then(function(newPages) {
            ...
            var originalMinHeight = root.css('min-height');

            root.css('transition', 'min-height 0s, max-height 0s');
            root.css('min-height', height + 'px');
            root.css('max-height', height + 'px');
            ...
            for (var i = 0; i < pagesToShow.length; i++) {
                pagesToShow[i].removeClass('hidden');
            }

            var scrollHeight = root[0].scrollHeight;
            var newMaxHeight = height < scrollHeight ? scrollHeight : height;

            // For some reason we need to make this async for it to work...
            setTimeout(function() {
                root.css('transition', 'min-height .2s linear, max-height .2s linear');
                root.css('min-height', originalMinHeight);
                root.css('max-height', newMaxHeight + 'px');
            }, 20);
