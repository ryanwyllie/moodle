...
define(
     ...
     */
    var showPages = function(root, pagesData, renderPagesContentCallback, eventElement) {
        var existingPages = [];
        var newPageData = [];
        ...
        if (newPageData.length && typeof renderPagesContentCallback === 'function') {
            ...
            var promises = renderPagesContentCallback(newPageData, {
                allItemsLoaded: function(lastPageNumber) {
                    eventElement.trigger(PagedContentEvents.ALL_ITEMS_LOADED, [lastPageNumber]);
                }
            });
        ...
        .then(function() {
            ...
            eventElement.trigger(PagedContentEvents.PAGES_SHOWN, [pagesData]);
        })
        .fail(Notification.exception)
        .always(function() {
    ...
    var init = function(root, eventElement, renderPagesContentCallback) {
        ...
        eventElement.on(PagedContentEvents.SHOW_PAGES, function(e, pagesData) {
            showPages(root, pagesData, renderPagesContentCallback, eventElement);
        });

        eventElement.on(PagedContentEvents.SET_ITEMS_PER_PAGE_LIMIT, function() {
