...
define(
     ...
     */
    var showPages = function(root, pagesData, pubSub, renderPagesContentCallback) {
        var existingPages = [];
        var newPageData = [];
        ...
        if (newPageData.length && typeof renderPagesContentCallback === 'function') {
            ...
            var promises = renderPagesContentCallback(newPageData, {
                allItemsLoaded: function(lastPageNumber) {
                    pubSub.publish(PagedContentEvents.ALL_ITEMS_LOADED, lastPageNumber);
                }
            });
        ...
        .then(function() {
            ...
            pubSub.publish(PagedContentEvents.PAGES_SHOWN, pagesData);
            return;
        })
        .fail(Notification.exception)
        .always(function() {
     ...
     * @param {function} renderPagesContentCallback Render pages content.
     */
    var init = function(root, pubSub, renderPagesContentCallback) {
        root = $(root);

        pubSub.subscribe(PagedContentEvents.SHOW_PAGES, function(pagesData) {
            showPages(root, pagesData, pubSub, renderPagesContentCallback);
        });

        pubSub.subscribe(PagedContentEvents.SET_ITEMS_PER_PAGE_LIMIT, function() {
