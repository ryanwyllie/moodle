...
function(
    ...
    var DEFAULT = {
        ...
        TEMPLATE_CONTEXT: {
            ...
            skipjs: true,
            disablecontrolwhileloading: false,
            controlplacementbottom: false
        },
        PAGING_BAR_TEMPLATE_CONTEXT: {
    ...
    var buildPagingBarTemplateContextKnownLength = function(numberOfItems, itemsPerPage) {
        ...
        var context = DEFAULT.PAGING_BAR_TEMPLATE_CONTEXT;
     ...
     * of pages to show.
     *
     * @param  {int|null} numberOfPages How many pages to have in the paging bar.
     ...
     * @return {object} The template context.
     */
    var buildPagingDropdownTemplateContext = function(numberOfItems, itemsPerPage, config) {
        if (isNull(itemsPerPage)) {
            itemsPerPage = DEFAULT.ITEMS_PER_PAGE_SINGLE;
        }
        ...
        var totalItems = 0;
        var lastIncrease = 0;
        // If given a number of items then use that to calculate the max pages
        // otherwise use the default.
        var maxPages = numberOfItems ? calculateNumberOfPages(numberOfItems, itemsPerPage) : DEFAULT.MAX_PAGES;
    ...
    var buildTemplateContext = function(numberOfItems, itemsPerPage, config) {
        var context = DEFAULT.TEMPLATE_CONTEXT;

        if (config.hasOwnProperty('disableControlWhileLoading')) {
            context.disablecontrolwhileloading = config.disableControlWhileLoading;
        }
        ...
        if (config.hasOwnProperty('dropdown') && config.dropdown) {
            context.pagingdropdown = buildPagingDropdownTemplateContext(numberOfItems, itemsPerPage, config);
        } else {
     ...
     */
    var createFromAjax = function() {
        var numberOfItems = null,
            itemsPerPage = null,
            renderPagesContentCallback = null,
            config = {};

        switch(arguments.length) {
            case 1:
                // renderPageContentCallback. 
                renderPagesContentCallback = arguments[0];
                break;
            case 2:
                // renderPageContentCallback, config.
                renderPagesContentCallback = arguments[0];
                config = arguments[1];
                break;
            case 3:
                // itemsPerPage, renderPageContentCallback, config.
                itemsPerPage = arguments[0]
                renderPagesContentCallback  = arguments[1];
                config = arguments[2];
                break;
            default:
                // numberOfItems, itemsPerPage, renderPageContentCallback, config.
                numberOfItems = arguments[0]
                itemsPerPage = arguments[1]
                renderPagesContentCallback  = arguments[2];
                config = arguments[3];
                break;
        }
