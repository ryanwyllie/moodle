...
function(
    ...
    /**
     * Enhance a pre-rendered a paged content widget where subsequent pages will be loaded
     ...
     *
     * @param  {object} root Callback for loading and rendering the items.
     * @param  {function} renderPagesContentCallback  Callback for loading and rendering the items.
     * @param  {object} config  Configuration options provided by the client.
     */
    var enhance = function(root, renderPagesContentCallback, config) {
        root = $(root);

        if (!root.is(PagedContent.rootSelector)) {
            root = root.find(PagedContent.rootSelector).first();
        }
        ...
        PagedContent.init(root, renderPagesContentCallback, id);
    ...
    var createWithTotalAndLimit = function(numberOfItems, itemsPerPage, renderPagesContentCallback, config) {
        ...
        Templates.render(TEMPLATES.PAGED_CONTENT, templateContext)
            .then(function(html, js) {
                ...
                enhance(html, renderPagesContentCallback, config);
