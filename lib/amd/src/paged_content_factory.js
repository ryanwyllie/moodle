...
function(
    ...
    var createFromAjax = function() {
        ...
        // This function takes a variable number of arguments. All of them are
        // optional except for the callback.
        for (var i = 0; i < arguments.length; i++) {
            var arg = arguments[i];

            if (typeof arg === 'function') {
                // Found the callback.
                renderPagesContentCallback = arg;
            } else {
                if (renderPagesContentCallback !== null) {
                    // We've found the callback so the only arg that can
                    // follow it is the config.
                    config = arg;
                } else {
                    // We haven't seen the callback yet so populate the item
                    // variables.
                    if (itemsPerPage === null) {
                        // Assume we've only got itemsPerPage for now.
                        itemsPerPage = arg;
                    } else {
                        // We mistook the previous arg for the itemsPerPage
                        // value so we need to correct it.
