...
define([ 'core/mustache',
     ...
     * @return {string}
     */
    var partialHelper = function(partialString) {
        var template = '';
        var name = '';
        var context = {};

        if (partialString.indexOf(' ')) {
            var index = partialString.indexOf(' ');
            name = partialString.substr(0,index).trim();
            contextJSON = partialString.substr(index).trim();
