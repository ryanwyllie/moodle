...
define(['jquery', 'core/key_codes'], function($, keyCodes) {
    ...
    /**
     * Trigger the custom event for the given browser event.
     *
     * This function will only fire the custom event if one hasn't already been
     * fired for the jQuery event. This prevents multiple event handlers being
    ...
    var triggerEvent = function(eventName, e) {
        if (!e.hasOwnProperty('originalEvent')) {
            // This is an even generated from JavaScript not a browser event so
            // we need to build the cache key for the event.
            var key = "" + eventName + e.type + e.timeStamp;

            if (!triggeredEvents.hasOwnProperty(key)) {
        ...
        if (!e.originalEvent.hasOwnProperty('customEventTriggered')) {
            ...
            e.originalEvent.customEventTriggered = true;
            $(e.target).trigger(eventName, [{originalEvent: e}]);
            return;
        }
    };
