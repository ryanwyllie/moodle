...
define(
     ...
     * @param {object} pubSub A PubSub instance.
     */
    var showPage = function(root, pageNumber, pubSub) {
        ...
        // a time.
        pubSub.publish(PagedContentEvents.SHOW_PAGES, [{
     ...
     * @param {object} pubSub A PubSub instance.
     */
    var registerEventListeners = function(root, pubSub) {
        ...
        root.on(CustomEvents.events.activate, SELECTORS.PAGE_ITEM, function(e, data) {
            ...
            if (!page.hasClass('disabled')) {
                ...
                showPage(root, pageNumber, pubSub);
                loading = true;
            }
        });
        ...
        // head of time.
        pubSub.subscribe(PagedContentEvents.ALL_ITEMS_LOADED, function(pageNumber) {
            loading = false;
        ...
        // This even is fired after all of the requested pages have been rendered.
        pubSub.subscribe(PagedContentEvents.PAGES_SHOWN, function() {
            // All pages have been shown so turn off the loading flag.
            loading = false;
        });

        // This is triggered when the paging limit is modified.
        pubSub.subscribe(PagedContentEvents.SET_ITEMS_PER_PAGE_LIMIT, function(limit) {
            // Update the limit.
            setLimit(root, limit);
            setLastPageNumber(root, 0);
            setActivePageNumber(root, 0);
            // Reload the data from page 1 again.
            showPage(root, 1, pubSub);
        });
    };
