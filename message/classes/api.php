...
class api {
    ...
    public static function search_messages($userid, $search, $limitfrom = 0, $limitnum = 0) {
        ...
        if ($users = $DB->get_records_sql($sql, $params, $limitfrom, $limitnum)) {
            foreach ($users as $user) {
                // Currently the id is set to the message id, so grab it.
                $messageid = $user->id;
                // Before we pass the object to create_contact() we should set the user id.
                $prefix = 'userfrom_';
                if ($userid == $user->useridfrom) {
                    $prefix = 'userto_';
                }
                // Set a unique identifier for the message.
                $user->searchmessageid = $messageid . $user->isread;
                $contacts[] = \core_message\helper::create_contact($user, $prefix);
            }
        }
     ...
     * @return \core_message\output\messagearea\people_search_results
     */
    public static function search_people($userid, $search, $limitnum) {
        global $CFG, $DB;
