...
function(
    ...
    Templates,
    MessageRepository,
    MessageDrawerViewConversationConstants,
    MessageDrawerEvents,
    LazyLoadList
) {
    ...
    var render = function(contentContainer, requests) {
        var formattedRequests = requests.map(function(request) {
            var privateConversations = request.conversations.filter(function(conversation) {
                return conversation.type == MessageDrawerViewConversationConstants.CONVERSATION_TYPES.PRIVATE;
            });
            var conversationId = privateConversations.length ? privateConversations[0].id : null;
            return {
                id: request.id,
    ...
    var load = function(listRoot, userId) {
        return MessageRepository.getContactRequests(userId, true)
    ...
    var handleContactRequestProcessed = function(root) {
        return function(request) {
            root.find('[data-request-id="' + request.id + '"]').remove();
