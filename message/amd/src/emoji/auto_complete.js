...
const render = async (root, shortNames) => {
    ...
    const html = await renderTemplate('core_message/emoji/auto_complete', renderContext);
    root.innerHTML = html;
};
...
const getWordFromPosition = (text, position) => {
    const startMatches = text.slice(0, position).match(/([:\w]*)$/);
    const endMatches = text.slice(position).match(/^([:\w]*)/);
    let startText = '';
    let endText = '';
 ...
 * @return {Bool}
 */
const isCompleteShortName = text => /^:\w+:$/.test(text);
 ...
 * @return {Bool}
 */
const isPartialShortName = text => /^:\w*$/.test(text);
