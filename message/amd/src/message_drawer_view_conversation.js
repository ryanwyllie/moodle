...
function(
    ...
    var render = null;
    var loggedInUserProfilePromise = null;
    ...
    var getLoggedInUserProfile = function(body) {
        if (!loggedInUserProfilePromise) {
            var userId = body.attr('data-user-id');
            loggedInUserProfilePromise = Repository.getProfile(userId, userId);
        }

        return loggedInUserProfilePromise;
    };
     ...
     * @param  {Number} memberLimit The number of members to include.
    ...
    var loadNewConversation = function(
        ...
        return render(newState)
            ...
            .then(function(conversation) {
                ...
                var newState = updateStateFromConversation(conversation);
                newState = StateManager.setLoadingMembers(newState, false);
                newState = StateManager.setLoadingMessages(newState, false);
                return render(newState)
                    .then(function() {
    ...
    var loadExistingConversation = function(
        ...
        return render(newState)
            .then(function() {
                return loadMessages(conversation.id, messageLimit, offset, newestFirst);
            })
            .then(function(messages) {
                setMessagesOffset(offset + messageLimit);
                return messages;
            })
            .then(function() {
        ...
        return render(newState)
            .then(function() {
                if (!conversationId && viewState.type == CONVERSATION_TYPES.PRIVATE) {
                    ...
                    return Repository.sendMessage(otherUserId, text);
                }
            })
            .then(function(result) {
            ...
            .then(function(result) {
                newConversationId = parseInt(result.conversationId, 10);
                return {
    ...
    var handleSelectMessage = function(e, data) {
        ...
        if (selection.toString() == '') {
            // Only toggle if we've got a click not if the user is selecting
            // text to copy.
    ...
    var show = function(header, body, footer, conversationOrId, action, otherUserId) {
        ...
        if (typeof conversationOrId == 'object') {
            var conversation = conversationOrId;
            var conversationId = parseInt(conversation.id, 10);
        } else {
            var conversation = null;
            var conversationId = parseInt(conversationOrId, 10);
        ...
        if (isNewConversation) {
            ...
            // conversation.
            getLoggedInUserProfile(body)
                .then(function(profile) {
                    if (conversation) {
                        return resetByConversation(body, conversation, profile, otherUserId);
                    } else if (conversationId) {
                        return resetById(body, conversationId, profile, otherUserId);
                    } else {
                        return resetNoConversation(body, profile, otherUserId);
                    }
                })
                .then(function() {
            ...
            switch(action) {
                case 'block':
                    requestBlockUser(otherUserId);
                    break;
                case 'unblock':
                    requestUnblockUser(otherUserId);
                    break;
                case 'add-contact':
                    requestAddContact(otherUserId);
                    break;
                case 'remove-contact':
                    requestRemoveContact(otherUserId);
                    break;
            }
        }
    };
