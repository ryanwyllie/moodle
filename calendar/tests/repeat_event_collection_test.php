...
class core_calendar_repeat_event_collection_testcase extends advanced_testcase {
    ...
    public function test_no_parent_collection() {
        ...
        $parentid = 123122131;
        $factory = new event_test_factory();
        // Event collection with a parent id that doesn't exist.
        $this->expectException(no_repeat_parent_exception::class);
    ...
    /**
     * Test that a collection will values behaves correctly.
     */
    public function test_values_collection() {
        ...
        foreach ($iterator as $index => $event) {
            $name = $event->get_name();
     ...
     * @return event
     */
    private function create_event($properties = []) {
        $record = new \stdClass();
...
/**
 * A test factory that will create events.
 */
class event_test_factory implements event_factory_interface {
