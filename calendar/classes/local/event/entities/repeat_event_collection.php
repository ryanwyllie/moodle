...
final class repeat_event_collection implements event_collection_interface {
    ...
    public function get_num() {
        ...
        return $this->num = isset($this->num) ? max($this->num, 0)
                          : max(($DB->count_records('event', ['repeatid' => $this->parentid]) - 1), 0);
    }
        ...
        foreach ($this->load_event_records() as $eventrecords) {
            foreach ($eventrecords as $eventrecord) {
                ...
                yield $this->factory->create_instance(
                    ...
                    !empty($eventrecord->description) ? $eventrecord->description : $parentrecord->description, //descriptionvalue
                    !empty($eventrecord->format) ? $eventrecord->format : $parentrecord->format, //descriptionformat
                    ...
                    !empty($eventrecord->instance) ? $eventrecord->instance : $parentrecord->instance, //moduleinstance
                    !empty($eventrecord->eventtype) ? $eventrecord->eventtype : $parentrecord->eventtype, //type
     ...
     * @return \stdClass 
     */
    private function get_parent_record() {
        global $DB;
